...
// Universally grant more breathing room for the larger font
  text_phone_battery_layer = text_layer_create( GRect(4, s_batt_top - 3, 64, s_batt_height + 4) );
  
  // Force the bold 18-point font across all watch models
...
void in_weather_handler(DictionaryIterator *received, void *context) {
    Tuple *appkey     = dict_find(received, MESSAGE_KEY_weather_temp);
    if (appkey != NULL)     { weather.current = appkey->value->int16; }
    appkey = dict_find(received, MESSAGE_KEY_weather_cond);
